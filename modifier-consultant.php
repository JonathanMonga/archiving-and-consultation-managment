  ...
  <div class="large-12 columns">
	  <?php
		...
		$modifier = false;
	  	...
	  	?>
	  <h1>Visualiser / ajouter un consultant</h1>
	  <p>Depuis cette interface vous pouvez visualiser ou modifier des consultants.</p>
...
<div class="row">
	<div class="large-4 columns text-right">
		<label class="inline">Sélectionner un consultant</label>
	</div>
	<div class="large-6 pull-2 columns">
		<select id="med-dropdown">
			...
			foreach ($conn->query($sql) as $row) {
			  print '<option value="' . $row['mat_consult'] . '">' . $row['nom_consult'] . ' ('.$row['prom_consult']. ')'; // Fill out all medicaments in a drop-down
			}
			?>
		</select>
	</div>
	...
	function ajaxLoadMed() {
		if ($('#med-dropdown').val() != "blanc") {
			...
			if (dropdown_id.length > 0 && dropdown_id != "nouveau" ) {			
				$('#detail').hide().load('ajax/ajouter-consultant.php?id='+ dropdown_id ).fadeIn();
			} else {
				$('#detail').hide().load('ajax/ajouter-consultant.php').fadeIn();
			}
		} else { 
			$('#detail').fadeOut();
		}
