  ...
  <div class="large-12 columns">
	  <?php
		...
		$modifier = false;
	  	...
	  	?>
	  <h1>Visualiser / ajouter un bibliothécaire</h1>
	  <p>Depuis cette interface vous pouvez visualiser ou modifier des bibliothecaire.</p>
...
<div class="row">
	<div class="large-4 columns text-right">
		<label class="inline">Sélectionner un biblio</label>
	</div>
	<div class="large-6 pull-2 columns">
		<select id="med-dropdown">
			...
			foreach ($conn->query($sql) as $row) {
			  print '<option value="' . $row['mat_biblio'] . '">' . $row['nom_biblio'] . ' ('.$row['mat_biblio']. ')'; // Fill out all medicaments in a drop-down
			}
			?>
		</select>
	</div>
	...
	function ajaxLoadMed() {
		if ($('#med-dropdown').val() != "blanc") {
			...
			if (dropdown_id.length > 0 && dropdown_id != "nouveau" ) {			
				$('#detail').hide().load('ajax/ajouter-biblio.php?id='+ dropdown_id ).fadeIn();
			} else {
				$('#detail').hide().load('ajax/ajouter-biblio.php').fadeIn();
			}
		} else { 
			$('#detail').fadeOut();
		}
