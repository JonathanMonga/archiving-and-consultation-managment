  ...
  <div class="large-12 columns">
	  <?php
		...
		$modifier = false;
	  	...
	  	?>
	  <h1>Visualiser / ajouter un archive</h1>
	  <p>Depuis cette interface vous pouvez visualiser ou modifier des archives.</p>
...
<div class="row">
	<div class="large-4 columns text-right">
		<label class="inline">Sélectionner un archive</label>
	</div>
	<div class="large-6 pull-2 columns">
		<select id="med-dropdown">
			...
			<?php
			$sql = 'SELECT `code_archive`, `sujet_travail`, `annee` FROM archives';
			foreach ($conn->query($sql) as $row) {
			  print '<option value="' . $row['code_archive'] . '">' . $row['sujet_travail'] . ' ('.$row['annee']. ')'; // Fill out all medicaments in a drop-down
			}
			?>
		</select>
	</div>
	...
	function ajaxLoadMed() {
		if ($('#med-dropdown').val() != "blanc") {
			...
			if (dropdown_id.length > 0 && dropdown_id != "nouveau" ) {			
				$('#detail').hide().load('ajax/ajouter-archive.php?id='+ dropdown_id ).fadeIn();
			} else {
				$('#detail').hide().load('ajax/ajouter-archive.php').fadeIn();
			}
		} else { 
			$('#detail').fadeOut();
		}
