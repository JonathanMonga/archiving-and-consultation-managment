  ...
  <div class="large-12 columns">
	  <?php
		...
		$modifier = false;
	  	...
	  	?>
	  <h1>Visualiser / ajouter un consultation</h1>
	  <p>Depuis cette interface vous pouvez visualiser ou modifier des consultations.</p>
...
<div class="row">
	<div class="large-4 columns text-right">
		<label class="inline">Sélectionner une consultation</label>
	</div>
	<div class="large-6 pull-2 columns">
		<select id="med-dropdown">
			...
			<?php
			$sql = 'SELECT * FROM liste_consultation, consultant WHERE liste_consultation.mat_consult = consultant.mat_consult';
			foreach ($conn->query($sql) as $row) {
			  print '<option value="' . $row['id'] . '">' . $row['nom_consult'] . ' ('.$row['date_consult']. ')'; // Fill out all medicaments in a drop-down
			}
			?>
		</select>
	</div>
	...
	function ajaxLoadMed() {
		if ($('#med-dropdown').val() != "blanc") {
			...
			if (dropdown_id.length > 0 && dropdown_id != "nouveau" ) {			
				$('#detail').hide().load('ajax/ajouter-consultation.php?id='+ dropdown_id ).fadeIn();
			} else {
				$('#detail').hide().load('ajax/ajouter-consultation.php').fadeIn();
			}
		} else { 
			$('#detail').fadeOut();
		}
